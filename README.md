...
| 기능 | 설명 |
|------|------|
| **BM25 검색** | 금융 키워드 필터 후 n-gram BM25 검색 → 유사도 ≥ 0.5면 데이터셋 답변 직접 반환 (LLM 미사용) |
| **RAG + Semantic Entropy** | 유사도 < 0.5면 BM25 검색 결과를 context로 Sonnet 답변 생성, Haiku × 5회 샘플링으로 신뢰도 판정 |
...
[BM25 n-gram 검색]
    │
    ├─ 유사도 ≥ 0.5 ──▶  [직접 반환]  데이터셋 답변 반환 (LLM 미사용)
    │
    └─ 유사도 < 0.5
            │
            ▼
        ...
        SE = −Σ P(c)·log P(c)  /  log(N)  ∈ [0, 1]
            │
            ├─ SE < 0.3  신뢰도 높음 ──▶  [RAG]  BM25 context + Sonnet 답변
            ├─ SE < 0.6  신뢰도 보통 ──▶  [RAG]  BM25 context + Sonnet 답변 + 상담 버튼
            └─ SE ≥ 0.6  신뢰도 낮음 ──▶         Sonnet 답변 (BM25 context 없음) + 상담 버튼
...
│   │   └── mcp_tools.py             # 도구 스키마 정의 (stub — 외부 API 미연결)
│   ├── core/
...
| 범위 | 신뢰도 | 처리 |
|------|--------|------|
| SE < 0.3 | 높음 | BM25 context + Sonnet 답변 |
| 0.3 ≤ SE < 0.6 | 보통 | BM25 context + Sonnet 답변 + 상담 버튼 |
| SE ≥ 0.6 | 낮음 | Sonnet 답변 (BM25 context 없음) + 상담 버튼 |
...
| `GET` | `/consult/stream/{session_id}` | 상담사 메시지 SSE 구독 |
| `POST` | `/consult/message/{session_id}` | 클라이언트 → 상담사 메시지 전송 |
| `POST` | `/consult/close/{session_id}` | 상담 세션 종료 |
